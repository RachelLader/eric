...
// Global Variables for THREE.JS
  ...
  function initBox(){
    ...
    //BACK WALL
    box_material2 = Physijs.createMaterial(
      ...
      for( var i = 0; i < fingers.length; i++ ){ 
        if( frame.fingers[i] ){ 
          ...
          for( var j = 0; j < finger.points.length; j++ ){
            ...
            //console.log(position, finger.points[j].position)
            ...
            finger.points[j].setLinearVelocity(new THREE.Vector3(0,0,0))
          }
        }else{ // If there aren't fingers, move them offscreen)
          var finger  = fingers[i];
          for( var j = 0; j < finger.points.length; j++ ){
            finger.points[j].position.x = 40,000;
            finger.points[j].position.y = 40,000;
            finger.points[j].position.z = 40,000;
          }
        }
      }
    }
